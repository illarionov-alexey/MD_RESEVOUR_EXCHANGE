...
class TrrReader:
    def __init__(self, trr, gro):
        ...
        print(f"Coordinates: type={type(traj.xyz[0])} shape={traj.xyz[0].shape} data_type={traj.xyz[0].dtype}" )
        self.traj = traj
    def get_frame_coords(self,frame_index):
        if 0<= frame_index and frame_index < self.traj.n_frames:
            return self.traj.xyz[frame_index]
...
def main():
    ...
    if partner_rank != -1: 
        ...
        trr_reader = TrrReader(config.reservoir_trr_file,config.reservoir_gro_file)
        for i in range(num_iter):
            time.sleep(config.reservoir_sending_interval)  # Small delay between sends
            data = trr_reader.get_frame_coords(np.random.randint(0, trr_reader.get_num_frames()))
            print(f"Reservoir with rank {rank} sending data with shape {data.shape} (count: {i + 1}/{num_iter})")
